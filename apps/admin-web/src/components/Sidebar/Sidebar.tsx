...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange }) => {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} transition-all duration-300 ease-in-out shrink-0 bg-navy-sidebar flex flex-col justify-between py-6 h-screen relative group`}>
      {/* 折叠切换按钮 */}
      <button
      ...
      <div className="flex flex-col gap-8 px-4">
        ...
        <nav className="flex flex-col gap-1.5">
          {MENU_ITEMS.map((item) => (
            ...
            >
              <span className="material-symbols-outlined !text-[26px] shrink-0">{item.icon}</span>
              {!isCollapsed && (
      ...
      <div className="flex flex-col gap-1.5 px-4 pt-4 border-t border-white/10">
        ...
        >
          <span className="material-symbols-outlined !text-[26px] shrink-0">settings</span>
        ...
        >
          <span className="material-symbols-outlined !text-[26px] shrink-0">logout</span>
